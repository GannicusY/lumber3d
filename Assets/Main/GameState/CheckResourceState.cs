...
namespace Main.GameState
{
	...
	public class CheckResourceState : FSMState<GameStateContext>
	{
		...
		/// <param name="remainingTime">剩余时间 KB</param>
		...
		private void OnResourceUpdateCallback(float progress, double size,double speed, float remainingTime)
		{
			...
			_loadingEventArgs.Tips = $"资源下载中({curSizeMb}/{totalSizeMb}mb, 下载速度{speed}kb/s, 剩余时间{remainingTime}秒";
			_loadingEventArgs.Progress = progress;
		...
		private void OnDownloadError(string localPath,string error)
		{
			...
			_loadingEventArgs.Tips = "Download fail, Network Error!";
			GameMode.Event.Trigger(this, _loadingEventArgs);
		}
