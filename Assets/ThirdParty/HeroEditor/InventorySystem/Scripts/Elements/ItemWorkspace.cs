...
namespace Assets.HeroEditor.InventorySystem.Scripts.Elements
{
    ...
    public abstract class ItemWorkspace : MonoBehaviour
    {
        ...
        protected void EquipItem(Item item, ItemContainer container, int amount = 1, string currencyId = null)
        {
            AddItemSilent(item, container, amount);
            
            var moved = container.Items.Last(i => i.Hash == item.Hash);
            SelectedItem = moved;
            
            Refresh();
            container.Refresh(SelectedItem);
        }

        public void AddItemSilent(Item item, ItemContainer container, int amount = 1)
        {
            if (item.Count <= 0) throw new ArgumentException("item.Count <= 0");
            if (amount <= 0) throw new ArgumentException("amount <= 0");
            ...
            {
                var target = container.Items.SingleOrDefault(i => i.Hash == item.Hash);

                if (target == null)
                {
                    container.Items.Add(new Item(item.Id, item.Modifier, amount));
                }
                else
                {
                    target.Count += amount;
        ...
        public void RemoveItemSilent(Item item, ItemContainer container, int amount = 1)
        {
            if (item.Count <= 0) throw new ArgumentException("item.Count <= 0");
            if (amount <= 0) throw new ArgumentException("amount <= 0");
            if (item.Count < amount) throw new ArgumentException("item.Count < amount");
