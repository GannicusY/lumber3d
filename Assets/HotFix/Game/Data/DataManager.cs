...
namespace _Game.Scripts.Data
{
    public class DataManager : BaseMonoSingleton<DataManager>
    {
        ...
        // --------- UserInfo ------------- //
        public long GetCurrencyAmount(ECurrency currencyType, int giftId = -1)
        {
            switch (currencyType)
            {
                case ECurrency.Coin:
                ...
                case ECurrency.Rose:
                    return UserInfo.rose;
                case ECurrency.Gift:
                    return UserInfo.giftDict.ContainsKey(giftId) ? UserInfo.giftDict[giftId] : 0;
            }

            Debug.Log($"currency type {currencyType} not defined yet!");
            return 0;
        }

        public long ObtainCurrency(ECurrency currencyType, int amount, int giftId = -1)
        {
            switch (currencyType)
            {
                case ECurrency.Coin:
                    ...
                    return UserInfo.coin;
                case ECurrency.Rose:
                    UserInfo.rose += amount;
                    return UserInfo.rose;
                case ECurrency.Gift:
                    if (!UserInfo.giftDict.ContainsKey(giftId))
                        UserInfo.giftDict[giftId] = amount;
                    else
                        UserInfo.giftDict[giftId] += amount;
                    return UserInfo.giftDict[giftId];
            }
            
            Debug.Log($"currency type {currencyType} not defined yet, return 0!");
            return 0;
        }
        
        public bool CostCurrency(ECurrency currencyType, int amount, int giftId = -1)
        {
            var curAmount = GetCurrencyAmount(currencyType, giftId);
            if (curAmount < amount)
            {
                Debug.Log($"currency:{currencyType} not enough, cur:{curAmount} required:{amount}!");
                return false;
            }

            ObtainCurrency(currencyType, -amount, giftId);
            return true;
        }
        // --------- UserInfo -------------- //
