...
namespace Wanderer.GameFramework
{
    public class AddressablesAssetsHelper : IAssetsHelper
    {
        ...
        private Dictionary<string, AsyncOperationHandle> _objectAsyncHandle = new();
        ...
        public void Clear()
        {
            ...
            // }
            // _objectAsync.Clear();
            foreach (var item in _objectAsyncHandle)
            {
                Addressables.Release(item.Value);
            }
            _objectAsyncHandle.Clear();
        ...
        public void LoadAsset<T>(string assetName, Action<T> callback) where T : UnityEngine.Object
        {
           Addressables.LoadAssetAsync<T>(assetName).Completed += (handle)=> {
               ...
               CheckAssetHandle(assetName, handle);
        ...
        public void UnloadAsset(string assetName)
        {
            ...
            if (_objectAsyncHandle.ContainsKey(assetName))
            {
                var handle = _objectAsyncHandle[assetName];
                _objectAsyncHandle.Remove(assetName);
                // if (handle != null)
                {
        ...
        private void CheckAssetHandle(string assetName, AsyncOperationHandle handle)
        {
            if (!_objectAsyncHandle.ContainsKey(assetName))
            {
                _objectAsyncHandle.Add(assetName, handle);
            }
            else
            {
