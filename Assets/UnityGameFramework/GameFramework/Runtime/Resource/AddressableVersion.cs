...
namespace Wanderer.GameFramework
{
    ...
    {
        static readonly Dictionary<string, string> RuntimeBuildTargetMapping = new()
        {
            {RuntimePlatform.OSXEditor.ToString(), "StandaloneOSX"},
            {RuntimePlatform.OSXPlayer.ToString(), "StandaloneOSX"},
            {RuntimePlatform.WindowsPlayer.ToString(), "StandaloneWindows"},
            {RuntimePlatform.WindowsEditor.ToString(), "StandaloneWindows"},
            {RuntimePlatform.IPhonePlayer.ToString(), "iOS"},
            {RuntimePlatform.Android.ToString(), "Android"},
            {RuntimePlatform.LinuxPlayer.ToString(), "StandaloneLinux64"},
            {RuntimePlatform.LinuxEditor.ToString(), "StandaloneLinux64"},
            {RuntimePlatform.WebGLPlayer.ToString(), "WebGL"},
            {RuntimePlatform.WSAPlayerX86.ToString(), "WSAPlayer"},
            {RuntimePlatform.WSAPlayerX64.ToString(), "WSAPlayer"}
        };
        ...
        public override async void CheckUpdate(Action<bool> needUpdate)
        {
            ...
            if (Directory.Exists(_catalogPath))
            {
                try
                {
                    ...
                    Debug.Log($"delete catalog cache {_catalogPath} done!");
                }
                catch (Exception e)
                {
            ...
            string configURI = 
                $"{config["ConfigPath"]}/DefaultConfig.json";
            Log.Info(configURI);
            ...
            var targetBuild = RuntimeBuildTargetMapping.ContainsKey(Application.platform.ToString())
                ? RuntimeBuildTargetMapping[Application.platform.ToString()]
                : Application.platform.ToString();
            res.Version.resServerNewIp =
                $"{config["ResOfficialUpdatePath"]}/{targetBuild}";
